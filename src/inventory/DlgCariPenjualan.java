...
public class DlgCariPenjualan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Cari Penjualan Obat, Alkes & BHP Medis ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12), new java.awt.Color(0, 0, 0))); // NOI18N
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                ...
                Valid.MyReport("rptNotaApotek.jasper", "report", "::[ Nota Transaksi Farmasi (Penjualan Bebas) ]::",
                        ...
                        + "p.keterangan, (select sum(total) from detailjual WHERE nota_jual='" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim() + "') tagihan, p.ongkir ppn, "
                        + "(select sum(a.total)+b.ongkir from detailjual a INNER JOIN penjualan b on b.nota_jual=a.nota_jual WHERE a.nota_jual='" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim() + "') 'tagihan+ppn' "
                        ...
                        + "WHERE dj.nota_jual = '" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim() + "' ", param);
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
                        ...
                        try {
                            ...
                            if (rs.next()) {
                                if (rs.getString("status").equals("Sudah Dibayar")) {
                                    ...
                                    Sequel.queryu("delete from tampjurnal");
                                    Sequel.menyimpan("tampjurnal", "'" + Sequel.cariIsi("select Penjualan_Obat from set_akun") + "','PENJUALAN OBAT BEBAS','0','" + ttljual + "'", "Rekening");
                                    Sequel.menyimpan("tampjurnal", "'" + Sequel.cariIsi("select kd_rek from penjualan where nota_jual=?", tbDokter.getValueAt(j, 0).toString().trim()) + "','KAS DI TANGAN','" + ttljual + "','0'", "Rekening");
                                    jur.simpanJurnal(rs.getString("nota_jual"), Sequel.cariIsi("select current_date()"), "U", "PENJUALAN DI " + Sequel.cariIsi("select nm_bangsal from bangsal where kd_bangsal='" + rs.getString("kd_bangsal") + "'").toUpperCase());
                                    Sequel.mengedit("penjualan", "nota_jual=?", "status='Sudah Dibayar'", 1, new String[]{rs.getString("nota_jual")});
                                    Sequel.menyimpan("tagihan_sadewa", "'" + rs.getString("nota_jual") + "','" + rs.getString("no_rkm_medis") + "','" + rs.getString("nm_pasien") + "','-',concat('" + rs.getString("tgl_jual")
                                            + "',' ',CURTIME()),'Pelunasan','" + ttljual + "','" + ttljual + "','Sudah','" + akses.getkode() + "','-'", "No.Nota");
    ...
    private void tbDokterMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDokterMouseClicked
        if(tbDokter.getRowCount()!=0){
            ...
            NoNota.setText(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString()); 
            kdmem.setText("");
            nmmem.setText("");
            kdptg.setText("");
            nmptg.setText("");
